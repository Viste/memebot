...
- ⚖️ Иногда (`CRIMINAL_CODE_PROBABILITY`) «назначает» автору мема статью УК РСФСР 1926 года — только реальные статьи и только по поводу
- ⌨️ Комментарии стримятся через `sendMessageDraft` (Bot API 10.0+): сначала «думает…», потом текст печатается по мере генерации; меню команд по scope с `is_ephemeral`
- 👻 Служебные ответы в группе (отказы, /help, /banlist) — эфемерные, видны только автору запроса (Bot API 10.2+);
- 📅 «Мем дня»: пару раз в день бот сам постит в канал сгенерированный мем по мотивам одного из сегодняшних мемов чата (`CHANNEL_MEMES_PER_DAY`, 0 = выключить)
